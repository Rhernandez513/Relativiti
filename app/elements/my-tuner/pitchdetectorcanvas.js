(function() {
  var PitchDetectorCanvasDraw = function PitchDetectorCanvasDraw(canvas, stats, pitchDetector) {
    if (!pitchDetector || !pitchDetector.buffer) {
      return;
    }
...
​
    ...
    if (pitchDetector.correlations) {
      ...
      if (pitchDetector.options.minCorrelation || pitchDetector.options.minCorrelationIncrease) {
        len = Math.max(stats.worstPeriod,stats.bestPeriod + 1);
      } else {
...
​
      ...
      val = 256 - (stats.bestCorrelation - stats.worstCorrelation) * 256;
...
​
      ...
      val = 256 - 256 * stats.worstCorrelation;
      canvas.moveTo(start,val);
      canvas.lineTo(20 + stats.bestPeriod,val);
      // line best correlation
      val = 256 - 256 * stats.bestCorrelation;
      canvas.moveTo(start,val);
      canvas.lineTo(20 + stats.bestPeriod,val);
      // line worst period
      val = 20 + stats.worstPeriod;
      canvas.moveTo(val,0);
