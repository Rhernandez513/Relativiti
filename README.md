...
- cordova, a command line tool to build, deploy and manage Cordova-based applications.
...
```sh
cd polymer-cordova-starter-kit && npm install && bower install
```
...
[(gulpfile.js)](https://github.com/emoriarty/polymer-cordova-starter-kit/blob/master/gulpfile.js)
...
### gulp build
Builds your app and moves it to the www folder. Usually you don't run this command directly, but it will be implicitly run by build-related cordova tasks (as explained above).
```sh
gulp build
```
Note that the build will not complete if you have any **ESLint or jsonlint errors** in your code! Sometimes it's necessary to let the build run anyway. This option will also work for all the build-related cordova tasks!
